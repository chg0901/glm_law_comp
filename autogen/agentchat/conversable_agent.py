...
class ConversableAgent(LLMAgent):
    ...
    def __init__(
        ...
        self._is_termination_msg = (
            ...
            else (lambda x: "TERMINATE" in content_str(x.get("content"))) # x.get("content")) == "TERMINATE"
        )
    ...
    def _generate_code_execution_reply_using_executor(
        ...
        for message in reversed(messages_to_scan):
            ...
            if len(code_blocks) == 0 and "from" not in message['content']:
                continue
            elif len(code_blocks) == 0 and "from" in message['content']:
                code_blocks = self._code_executor.code_extractor.extract_code_blocks("```python\n\n" + message["content"] + "\n```")
