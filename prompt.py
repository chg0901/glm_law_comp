...
例四：如果问题是"帮我查询一下河南中敏传感器技术研究院有限公司的组织机构代码"
你可以返回如下代码块
```python
...
1.  如果问题可以被下方函数解决，请你先从functions模块中导入函数再解决任务，最后结果要使用print函数打印出来。
2.  请你把代码放在```python```代码框中，方便我执行你的代码，请把结果用print打印出来。
3.  关联上市公司股票简称都是三至四个字的，如果比四个字多就是关联上市公司全称。
...
可以使用的参数有[关联上市公司股票代码,关联上市公司股票简称 # 三至四字,关联上市公司全称]
例如：
    输入：
    key="关联上市公司全称"
    value="北京同仁堂股份有限公司"
...
get_max_law_office(is_plaintiff:bool, company_name: str) -> str
根据公司名获取该公司合作次数最多的律师事务所

例如：
    输入：
    is_plaintiff = True #是否为原告
    company_name = "九牧王股份有限公司"

    输出：
    与九牧王股份有限公司合作最多的事务所为福建达业律师事务所律师，共合作9次。
        或者：该公司作为原/被告时没有合作的律所
